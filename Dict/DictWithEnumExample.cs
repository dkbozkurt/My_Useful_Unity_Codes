...
namespace Creative.Scripts.Alexa
{
    ...
    public class DictWithEnumExample : MonoBehaviour
    {
        [Header("Camera")]
        ...
        private AlexaBlends _lastActivatedBlendShape = AlexaBlends.None;

        private float _blendIncrease= 0f;
        private float _blendDecrease= 100f;
        
        private Dictionary<AlexaBlends, int> _alexaBlendShapeDictionary = new Dictionary<AlexaBlends, int>
        {
            {AlexaBlends.None,0},
            {AlexaBlends.Smile,1},
            {AlexaBlends.Sad,2},
            {AlexaBlends.Suprize,3},
            {AlexaBlends.Blink,4}
        };
        ...
        private void Update()
        {
            ...
            {
                IncreaseBlendShapeValue(_alexaBlendShapeDictionary[AlexaBlends.Smile]);
            }

            if (Input.GetKeyDown(KeyCode.Keypad2))
            {
                IncreaseBlendShapeValue(_alexaBlendShapeDictionary[AlexaBlends.Sad]);
            }
            
            if (Input.GetKeyDown(KeyCode.Keypad3))
            {
                IncreaseBlendShapeValue(_alexaBlendShapeDictionary[AlexaBlends.Suprize]);
            }
            
            if (Input.GetKeyDown(KeyCode.Keypad4))
            {
                IncreaseBlendShapeValue(_alexaBlendShapeDictionary[AlexaBlends.Blink]);
            }
        ...
        {
            if( _lastActivatedBlendShape == AlexaBlends.None) return;
            ...
            {
                _lastActivatedBlendShape = AlexaBlends.None;
                // Debug.Log($"Last activated blend shape: {_lastActivatedBlendShape}");
            }
            else 
            {
